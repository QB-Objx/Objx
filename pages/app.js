  ...
  throughput: {
    ...
    rows: [
      ...
      },
      {
        ...
        highlight:
          "Current gap. OBJX is still behind eager fanout leaders here.",
        results: [
      ...
      },
      {
        ...
        highlight:
          "OBJX #1, ahead of the field in the end-to-end transaction scenario.",
        results: [
  ...
  transactions: {
    ...
    rows: [
      ...
      },
      {
        ...
        highlight:
          "Rollback is measurable too, not hidden behind aggregate numbers.",
        results: [
    ...
    rows: [
      {
        scenario: "Find by id",
        highlight:
          "Compile is tiny; real cost is still the database round trip.",
        results: [
          { orm: "Compile cache hit", value: 42016.81, className: "objx" },
          { orm: "Compile no cache", value: 49875.31, className: "knex" },
          {
            orm: "Session execute precompiled",
            value: 671.7,
            className: "sequelize",
          },
          {
            orm: "Session execute builder",
            value: 672.92,
            className: "typeorm",
          },
        ],
      },
      {
        scenario: "List page",
        highlight:
          "Cached compile is effectively noise next to query execution.",
        results: [
      ...
      },
      {
        scenario: "Update active",
        highlight:
          "Write-path work is where runtime optimization matters most now.",
        results: [
...
function createMiniRow(row) {
  ...
  const objx =
    row.results.find((item) => item.orm === "OBJX") ?? row.results[0];
...
function renderBenchmarkView(viewKey) {
  ...
  summary.replaceChildren(
    ...view.summary.map((item) => createSummaryCard(item)),
  );
